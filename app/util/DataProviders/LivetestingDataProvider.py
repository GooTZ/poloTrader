...
class LivetestingDataProvider(DataProvider):
	...
	def doTheLoop(self):
		...
		# Fetch all fast paced data from poloniex
		...
		if delta5Min > self.timePeriod.value:
			return # next iteration if not enough time passed
		...
		start = app.util.Timing.getHistoricTimestamp(minute = self.timePeriod.value)
		chart = self.polo.returnChartData("BTC_ETH", period = self.timePeriod.value, start = start, end = 9999999999)
